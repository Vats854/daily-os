...
const contracts = [
  ...
  ["calendar start and end reminders are independent", app.includes('data-calendar-block-field="reminderMinutes"') && app.includes('data-calendar-block-field="endReminderMinutes"') && app.includes("daily-os-block-start-") && app.includes("daily-os-block-end-")],
  ["notification permission stays user initiated", app.includes('data-notification-action="enable"') && app.includes('data-notification-action="test"') && !app.includes('block.reminderMinutes = value === "" ? null : Number(value);\n      if (value !== "") requestReminderPermission')],
  ["background push is wired", worker.includes('addEventListener("push"') && supabaseClient.includes("savePushSubscription") && app.includes("pushManager.subscribe")],
  ...
  ["quick task composer exposes plan, date, list, and priority", ["planBucket", "dueDate", "area", "priority"].every((field) => app.includes(`data-quick-task-field=\"${field}\"`))],
  ["task rows expose a direct Today/Inbox transition", app.includes("data-simple-plan-action") && app.includes("setTaskPlanBucket(item, target)")],
  ["Inbox requires an explicit outcome before object creation", ["create-task", "create-note"].every((outcome) => app.includes(`data-inbox-action="${outcome}"`)) && app.includes('data-inbox-choice="type"') && app.includes('data-inbox-triage-field="area"') && app.includes('data-inbox-triage-field="folderId"') && app.includes('status: "open"') && app.includes("const aiResult = await classifyInboxWithAi(text)") && !app.includes("Предложение ассистента") && !styles.includes(".simple-inbox-alternatives")],
  ...
  ["Today renders an accepted daily plan", app.includes("function renderTodayWorkspace") && ["focus", "reviewSummary"].every((field) => app.includes(`data-daily-plan-field="${field}"`)) && styles.includes(".today-plan-workbench")],
  ["Today keeps linked tasks out of the unscheduled list", app.includes("const linkedTaskIds = new Set") && app.includes("filter((item) => !linkedTaskIds.has(item.id))") && app.includes("data-today-task-id")],
  ["Today opens and creates calendar blocks in place", app.includes('data-today-action="select-block"') && app.includes('command === "add-block"') && app.includes('module === "tasks" && state.settings.activeView === "today"')],
  ...
  ["project mentor uses bounded AI context", app.includes("projectJourneyReviewContext") && app.includes('fetch("/api/ai/journey-review"') && app.includes("tasks: projectTasks.slice(0, 20)")],
  ["project mentor keeps stage confirmation explicit", app.includes("requestProjectMentorReview") && app.includes("proposeProjectStage(projectItem, review.proposedStage") && app.includes('data-simple-project-action="confirm"')],
  ["project mentor has an offline fallback", app.includes("localProjectMentorReview") && app.includes('provider: "local"') && styles.includes(".simple-project-mentor")],
  ["task decomposition uses bounded context", app.includes('fetch("/api/ai/task-decompose"') && app.includes("existingSubtasks: (taskItem.subtasks || []).slice(0, 12)")],
  ["task decomposition requires explicit confirmation", app.includes('data-simple-action="apply-decomposition"') && app.includes("requestTaskDecomposition") && app.includes("item.subtasks.push")],
  ["task decomposition has an offline fallback", app.includes("localTaskDecomposition") && app.includes("simple-decomposition-draft")],
